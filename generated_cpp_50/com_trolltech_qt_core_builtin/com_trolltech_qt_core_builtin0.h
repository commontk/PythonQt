...
{ Q_OBJECT
public:
Q_ENUMS(FormatType CurrencySymbolFormat QuotationStyle Language Script NumberOption Country MeasurementSystem )
...
{ Q_OBJECT
public:
Q_ENUMS(AspectRatioMode BGMode MouseButton SortOrder Orientation WidgetAttribute GestureState GlobalColor PenStyle NavigationMode GestureFlag ToolBarArea WindowFrameSection PenCapStyle SizeHint DayOfWeek WindowModality MatchFlag DockWidgetAreaSizes FindChildOption AnchorPoint CaseSensitivity TextFlag BrushStyle ItemSelectionMode DateFormat ApplicationAttribute ItemFlag ScrollBarPolicy WhiteSpaceMode TimerType DropAction FocusPolicy CursorShape TouchPointState CursorMoveStyle InputMethodHint ArrowType TextFormat HitTestAccuracy ToolButtonStyle FocusReason TextElideMode CoordinateSystem KeyboardModifier UIEffect ScreenOrientation ImageConversionFlag TransformationMode PenJoinStyle EventPriority LayoutDirection TextInteractionFlag Corner TileRule ConnectionType AlignmentFlag Key WindowState CheckState ToolBarAreaSizes SizeMode Axis FillRule GestureType ClipOperation InputMethodQuery TimeSpec ItemDataRole ShortcutContext MaskMode WindowType DockWidgetArea ContextMenuPolicy )
Q_FLAGS(MouseButtons Orientations GestureFlags ToolBarAreas MatchFlags ItemFlags DropActions TouchPointStates InputMethodHints KeyboardModifiers ImageConversionFlags TextInteractionFlags Alignment WindowStates WindowFlags DockWidgetAreas )
